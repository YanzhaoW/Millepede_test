...
void Mille::mille(const DataPoint& data_point)
{
    ...
    for (const auto [index, value] : rng::views::enumerate(data_point.locals) |
...
void Mille::write_to_binary()
{
    ...
    output_file_.write(reinterpret_cast<const char*>(buffer_.get_values().data()), get_size_in_bytes(buffer_.get_values()));
    output_file_.write(reinterpret_cast<const char*>(buffer_.get_indices().data()), get_size_in_bytes(buffer_.get_indices()));
}
...
void Mille::check_buffer_size(std::size_t nLocal, std::size_t nGlobal)
{
    if (buffer_.get_current_size() >= max_buffer_size_)
    {
        throw std::runtime_error(fmt::format(
            R"(Mille::checkBufferSize: Buffer too short ({}), \n need space for nLocal ({}) \nGlobal \({}\) local/global derivatives, {} already stored!)",
            max_buffer_size_,
            nLocal,
            nGlobal,
